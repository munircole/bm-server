...
module.exports = {
  ...
  Mutation: {
    postQuestion: async (_, args, context) => {
      ...
      const { title, department, courseName, courseCode, body, tags } = args;

      const { errors, valid } = questionValidator(title, department, courseName, courseCode, body, tags);
      if (!valid) {
    ...
    editQuestion: async (_, args, context) => {
      const loggedUser = authChecker(context);
      const { quesId, title,department, courseName, courseCode, body, tags } = args;
